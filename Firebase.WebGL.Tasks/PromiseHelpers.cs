...
{
    static uint PromiseID = 0;
    internal static uint NextPromiseID() => PromiseID++;
    /// <summary>
    ...
    public static IPromise<Tuple<T1, T2>> All<T1, T2>(IPromise<T1> p1, IPromise<T2> p2)
    {
        ...
        var promise = new TaskCompletionSource<Tuple<T1, T2>>();
        ...
        p1
            .Then(val =>
            {
                ...
                if (numUnresolved <= 0)
                {
                    promise.SetResult(Tuple.Create(val1, val2));
                }
            })
            .Catch(e =>
            {
